...
The default username is `admin`. For local development with `pnpm dev`, use `admin123`. Docker and production starts do not include a default user; if no `ADMIN_PASSWORD` environment variable is set on first launch, a random password is generated and printed to the server logs.

To pre-set the admin password:

```bash
# Via environment variable
ADMIN_PASSWORD=your-secure-password docker run -p 3000:3000 huangjunsen/xiaozhi-mcphub
```
